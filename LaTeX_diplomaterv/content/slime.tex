...
De az átláthatósághoz az is fontos volt, hogy a jelölések egyediek legyenek és így felismerhetőek az alárendelt kódban.
...
Mivel ezek a nevek törölhetőek, mindenki el is távolíthatja az általa bevezetett neveket, hogy ne szennyezze a többiek namespace-ét.
...
Bizonyos esetekben a parszer az ilyen meghivatkozást \texttt{Name} típusú változónak tévesztheti, például ha egy listát készítünk, és vesszővel felsoroljuk az elemeit (Lásd fejezet~\ref{sec:SContDecl}), akkor azt vesszők listájának tévesztheti.
...
\{\# Ez egy komment, ami nem jelenik meg a generált kódban \#\}
...
A nyelvben az egymásba ágyazott típusokat '<'-tel tagoljuk, így: List<List<Temp
Ez C++ nyelvekben template-eléssel így néz ki: List<List<Temp> >
A záró zárójelek használata viszont csak akkor praktikus, ha egy típusba több másik is be van ágyazva.
A Slime jelölése kiegészíthető zárójelezéssel, például ha bevezetjük majd a dictionary-ket Dict néven:
...
\item \emph{ Egy Cont változó iterátorához egy másikét adva az előbbi minden eleméhez hozzáadnja az utóbbi összes elemét:}
\begin{itemize}
\item \emph{változók:}
\begin{itemize}
\item \{= list1 : List<Temp  := \{|temp1:\{\$slot1\$\}|\}, \{|temp2:\{\$slot1\$\}|\}, \{|temp3:\{\$slot1\$\}|\}=\}
...
\item \emph{kimenet:}
List: \{|temp1:\{\$slot1\$\}|\}, \{|temp2:\{\$slot1\$\}|\}, \{|temp3:\{\$slot1\$\}|\}
\item \emph{magyarázat:}
Sajnálatos mód ugyan ilyenkor elvégzi a műveletet, de hibásan az előbbi listát téríti vissza.
Ez később még javítva lesz.
Egy másik elképzelés, hogy ilyenkor a két lista elemeit párosával illessze egymásba.
Ha azt megoldanánk, akkor a sablonba beszúrni kívánt adatok definiálását és beszúrását egy sorban el lehetne végezni.
\end{itemize}
...
A Name típus meg fog szűnni és implementálva lesz a plus művelet két operátoros esete.
...
Deklarációnál az adatokat egy darab fában is meg lehet majd adni, amit a művelet a várt típus függvényében dolgoz fel.
Így a többszörösen egymásba ágyazott deklarációs zárójeleket és rengeteg felesleges karaktert el lehet nómajd kerülni.
\item \emph{test:}
...
Mivel specialitásuk, a HTML általánosan hatékonyabbak is a Slime-nál.
...
Ebben a példában azt kívánom szemléltetni, hogy a kész adatok esetén, hogyan érhetőek el a Jade-hoz~\ref{sec:jade} hasonló technikák. A comf~\ref{sec:SStatesOfDevelopment} mérföldkő elérése után már itt is elkerülhetőek lesznek az egymásba ágyazott deklarációs zárójelek.\footnote{\url{https://github.com/Hollo1996/SlimeAnUTLE/blob/master/test_codes/html_example}}

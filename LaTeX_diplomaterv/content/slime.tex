...
\label{sec:SIntro}
%----------------------------------------------------------------------------
A szakdolgozatomban a Slime nyelv kidolgozásával foglalkoztam.
...
Ennek több oka volt. 
Először is az IRTG-nek a legtöbb hiányossága későbbi fejlesztés során megszűnhet a nyelv későbbi kiegészítésével (Lásd fejezet ~\ref{sec:sec:SEIRTG}).
...
Az ALTO fejlődésével sok kód feleslegessé válik. 
Ezért külső megoldás igényel. 
Minden nyelv fejlődik, de mindig lesznek fejletlen nyelvek is, amik felett a Slime hasznos lesz.

Programozási és adatleíró nyelveknek széles rétege alul fejlett. 
Ennek egyik válfaját alkotják azok a nyelvek, amik még fiatalok és egyszerűen nem jutottak még el a kellő érettségig. 
A másik típus pedig azok a nyelvek, amiknek túlzottan célorientáltak a bennük rejlő lehetőségekhez képest.
A fejlesztőknek sokszor nem éri meg az összes alapfunkcionalitást implementálni. 
...
A réteges architektúra és egyirányú viszony miatt a kiegészített nyelvre \textit{alárendelt nyelv}ként (subordinate language) fogok hivatkozni.
A templatek esetében megszokott terminológia ``master document''-ként hivatkozik az alárendelt nyelv sablonjaira.
Ezzel ellentétben mi \textit{alárendelt dokumentum}ként (subordinate document) fogok hivatkozni rá. 
...
További fejlesztéseket igényel
...
Ilyen nyelv az IRTG, HTML, XML és irónikusan annak az ANTLR-nek a \textit{lexer} és parser nyelvtan leíró nyelve, amit a Slime jelenlegi fordítója használ (lást Példák ~\ref{sec:SEIRTG}). 
...
Ecélból a nyelv külön fordítót, templatelést és szöktetést használ e célból.
...
A könnyen tanulhatóságot a beszédes, de tömör jelölések, kevés, de sok oldalú műveletek és következetes működés, valamint a kódok átláthatósága is segíti.
...
Ugyanakkor van lehetőség a csukó zárójelek esetenkénti elhagyására.
Sok nyelv az adatot elkülöníti a kódtól. 
Slimeban a kód tartalmazza a templatet, de külön fájlba szervezéssel elkülöníthetőek. 
Más templater engine-k könyvtáraival ellentétben nem a Slime-ba ágyazzuk az alárendelt nyelv kódját, hanem  
...
Avagy elkülöníti a logikát és a a sablonokat.
...
A hivatalos oldalánn is javascript library-nek hívják az \textit{Underscore Templates}-et.
Más alatt nem is elérhető és nem kíván teljes nyelv lenni.
Az Underscore nem csak templatelésre való.
Egy általános kiegészítése a javascriptnek.
...
A lehető legkevesebb új információ mellett lehet hatékonyan template-elni.
...
Ezeknek részletezése nem fér bele a szakdolgozat kereteibe.
Ezért csak a szintaxis szemléltetésének céljával sorolok fel alapvető nyelvi elemeket:
...
\item A második alfejezetben a típusok közös ősével fojtatom.
\item A harmadik alfejezetben a tároló típusok őséről írok.
\item A negyedik alfejezetben a sablonok szintaxisáról és alkotó elemeiről írok.
...
\item A tizeneggyedik fejezetben a haladó szintaxisról írok.
...
Például deklaráció során a létrehozott változó metaadatait leíró részt(nevek és típus) egy \textit{“:=”} zárja.
...
Inkább pár tabulátorral beljebb szeretnénk igazítani, hogy jól látszódjon a blokkon belül van.
...
Valamint ki lesz egészítve a karakterek legnépszerűbb nyelvekben használt neveivel is.
\end{itemize}
...
\label{sec:SContDecl}
%----------------------------------------------------------------------------
Mivel a nyitó és csukó zárójelek rakása körülményes, ezért elég korán be lettek vezetve a több műveletes zárójelek.
...
Nagyon problémás minden műveletnél egy nyitó és egy csukó zárójelet is rakni. 
...
Különben a kompakt zárójelek nagyon beleolvadnának a sablonokba.
...
%----------------------------------------------------------------------------
A fordító jelenlegi implementációja \textit{Kotlin}-ban\footnote{\url{https://kotlinlang.org}} készült.
...
A Slime implementáció ANTLR-t (ANother Tool for Language Recognition)\footnote{\url{https://www.antlr.org}} használ a parszoláshoz.
...
A tokenezéshez szüksége van egy \textit{lexer grammar}-re (SlimeLexer.g4), és a fa felépítéséhez egy \textit{\textit{parser grammar}}-re(SlimeParser.g4). 
...
A \textit{lexer} nyelvtan \textit{fragment}-ekből és \textit{lexer} rule-okból áll. 
A \textit{fragment}-ek önmagukban nem azonosítanak tokent, de részei lehetnek \textit{lexer} szabályoknak. 
...
A \textit{lexer} szabályok azonosítják a tokeneket
...
A \textit{lexer} képes váltogatni módok között, amikben más \textit{lexer} szabályok lesznek érvényesek. 
...
Például ilyen, hogy “\{|” esetén belépünk a Temp belsejében érvényes szintaxis \textit{lexer} módjába és “|\}” esetén pedig visszatérünk. 
A parser és \textit{lexer} nyelvtan össze is vonható, de vegyes nyelvtanban nem lehet a lexernek több módja.
...
Ugyanakkor a \textit{fragment}ek  minden mode alatt elérhetőek. 
Használjuk is őket, de nem jelent tömörítést, ha a “\{|” zárójelet egy minimum két karakteres nevű \textit{fragment}-be csomagoljuk.
...
Sőt template-eléssel a \textit{fragment}ek is teljes mértékben kiválthatóak.

A parser szabályok már a \textit{lexer} és más parser szabályokból építkeznek. 
...
A zárójel visit függvénye csak továbbítja hozzá tartozó body visit függvényének kimenetét. 
...
Mérföld kövekről van szó, nem verziókról, mivel az egyes fázisok között lehetnek időbeli átfedések.
...
A szöveget karakter-láncként fogjuk látni, és tudni fogjuk kezelni.
A templateket is szövegként fogjuk látni, ahol minden Spec és Slot egy karakternyi helyet foglal.
...
Avagy egy zárójelben lesznek a feltétel-, következménypárok.
...
Minden eddigi Vari osztály egyesítve lesz egy Vari osztályá.
...
\begin{itemize}
\item \emph{alge:}
A templatelés folyamata felfogható egy fa felépítéseként is, ahol a csúcsok a sablonok, és a csúcsoknak annyi gyermeke lehet, ahány mező van a sablonban.
...
Ez történhetne lebegő pontos és egészként is.
...
(lásd fájl \footnote{\url{https://github.com/Hollo1996/SlimeAnUTLE/blob/master/test_codes/irtg_example1}})
...
(lásd fájl 
\footnote{\url{https://github.com/Hollo1996/SlimeAnUTLE/blob/master/test_codes/irtg_example2}}
\footnote{\url{https://github.com/Hollo1996/SlimeAnUTLE/blob/master/test_codes/irtg_example2data}})
Fontos! Az első fájl a másodikat a Slime programhoz viszzonyított ../test\_codes/irtg\_example2data mappában keresi.
...
(lásd fájl \footnote{\url{https://github.com/Hollo1996/SlimeAnUTLE/blob/master/test_codes/irtg_example3}})
...
(lásd fájl \footnote{\url{https://github.com/Hollo1996/SlimeAnUTLE/blob/master/test_codes/irtg_example4}})

%----------------------------------------------------------------------------
\subsection{ANTLR}
\label{sec:SEANTLR}
%----------------------------------------------------------------------------
Az ANTLR esetében a legkézreesőbb példát hozom, a Slime \textit{lexer} nyelvtanát. 
...
(lásd file \footnote{\url{https://github.com/Hollo1996/SlimeAnUTLE/blob/master/test_codes/antlr_example}})
...
(lásd file \footnote{\url{https://github.com/Hollo1996/SlimeAnUTLE/blob/master/test_codes/html_example}})

Hiszek benne, hogy a Slime hamar versenyképes lesz más templater enginekkel szemben is.
...
%----------------------------------------------------------------------------
 ...
 (lásd file \footnote{\url{https://github.com/Hollo1996/SlimeAnUTLE/blob/master/test_codes/xml_example}})

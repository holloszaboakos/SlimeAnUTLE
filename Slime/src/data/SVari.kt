...
abstract class SVari(val typeName: String, _names: List<SName>) {
    ...
    protected val names: MutableMap<SVari, MutableList<SName>> = mutableMapOf()
    ...
    fun addNames(
        _names: List<SName>,
        owner: SFile? = DataContainer.focus
    ): SVari {
        if (_names.isNotEmpty() && owner!=null) {
            names[owner] = _names.toMutableList()
            for (n in _names)
